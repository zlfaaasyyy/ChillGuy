...
public class MusicAdapter extends RecyclerView.Adapter<MusicAdapter.ViewHolder> {
    ...
    public void setPlayingPaused() {
        isPaused = true;
        if (playingPosition != -1) notifyItemChanged(playingPosition);
    }

    public void setPlayingResumed() {
        isPaused = false;
        if (playingPosition != -1) notifyItemChanged(playingPosition);
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder h, int pos) {
        ...
        boolean isActive  = pos == playingPosition;
        boolean isPlaying = isActive && !isPaused;
        ...
        h.itemView.setBackgroundResource(isActive
                ? R.drawable.bg_music_card_active
                : R.drawable.bg_music_card);

        h.btnPlay.setOnClickListener(v -> {
            if (listener != null) listener.onPlayClick(track, pos);
        });
